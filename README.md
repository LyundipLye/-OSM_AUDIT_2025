...
- **`06_thermal_chart.py`** (Python): Processes raw Land Surface Temperature (LST) CSV data exported from Google Earth Engine (USGS Landsat 8 TIRS). The script tracks LST anomalies over a decade, applying a 365-day rolling mean and linear regression model to identify the structural thermal escalation caused by high-density server rendering and asphalt heat retention.
...
│   └── 06_thermal_chart.py     #   LST thermodynamic trend visualisation
├── data/
...
The GEE script (`scripts/04_gee_ndvi_pipeline.js`) is designed for native execution within the [Google Earth Engine Code Editor](https://code.earthengine.google.com/). Users require an authenticated Google Cloud Project (GCP) environment.
